...
from webtest.model.WebCollectionDetailModel import WebCollectionDetail  # 请替换为你的ApiModule模型类
from datetime import datetime
# 模块信息
module_name = "WebCollectionDetail"  # 模块名称
module_model = WebCollectionDetail
module_route = Blueprint(f"route_{module_name}", __name__)

# 其它模块
from webtest.model.WebInfoModel import WebInfo  # 请替换为你的ApiModule模型类
...
def queryByPage():
    ...
    try:
        ...
        with application.app_context():
            ...
            for data in datas:
                # 1. 通过用例的编号查询到具体的信息
                web_info_data = WebInfo.query.filter_by(id=data.web_info_id).first()
                # 2. 把查询到的用例信息(web_info_data) 和 当前的(data) 都加到result中
                result = respModel().get_custom_attributes(web_info_data)
